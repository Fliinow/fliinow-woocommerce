...
Fliinow redirects to successCallbackUrl or errorCallbackUrl
       ↓
Plugin verifies status and updates the order:
  - FAVORABLE/CONFIRMED → order completed ✓
  - PENDING → order on hold
  - ERROR/REFUSED → order cancelled
```
...
```bash
# PHP unit tests (106 tests, 218 assertions)
